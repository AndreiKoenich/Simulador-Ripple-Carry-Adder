...
void operacoes() /* Realiza as operacoes comum e com interferencia de CARRY IN entre os operandos binarios. */
{
    ...
    while (1)
    {
        ...
        switch(tecla)
        {
            ...
            case '6':
            {
                printf("RESULTADO TROCANDO AS PORTAS LOGICAS <XOR> POR PORTAS LOGICAS <OR> NO RCA:\n\n");
                analise_mudaporta(parametros,MUDANCA_OR);
                printf("\nPressione qualquer tecla para voltar ao menu anterior.\n");
                getch();
                break;
            }
            ...
            case '7':
            {
                printf("RESULTADO TROCANDO AS PORTAS LOGICAS <XOR> POR PORTAS LOGICAS <AND> NO RCA:\n\n");
                analise_mudaporta(parametros,MUDANCA_AND);
                printf("\nPressione qualquer tecla para voltar ao menu anterior.\n");
                getch();
                break;
            }

            case '8': /* Gera arquivo contendo resultados todas os cortes de CARRY IN de CARRY IN possiveis. */
            {
            ...
            case '9': /* Permite que o usuario realize novos operandos. */
                return;
                break;
...
void resultados_comum (Operacao parametros, int saida[]) /* Mostra os resultados da soma sem nenhum corte. */
{
    ...
    printf("%d", calcula_decimal(parametros.n1,parametros.bits));
    printf(" + ");
    printf("%d", calcula_decimal(parametros.n2,parametros.bits));
    printf(" = ");
    printf("%d", calcula_decimal(saida,parametros.bits));
}
...
void desprezo_cumulativo(Operacao parametros, int contador_carry) /* Realiza as somas com cortes cumulativos de carry out. */
{
    ...
    for (int i = 1; i <= contador_carry; i++)
    {
        ...
        erro_relativo(novo_decimal,decimal_somacomum);
    }
}
...
void despreza_tudo(Operacao parametros) /* Realiza as somas desprezando todos os CARRY IN. */
{
    ...
    saida_comum(parametros,saida_normal); /* Determina a soma entre os dois operandos sem cortes, retornando no vetor de saida. */
    ...
    printf("Erro relativo:\t");
    erro_relativo(novo_decimal,decimal_normal);
}
...
void recebe_especificos(int cortes[], int vetor_carry[],int contador_carry, int bits) /* Recebe do usuario os bits nos quais ocorrerao interferencia de CARRY IN. */
{
    ...
    printf("Ocorreram CARRY IN nos seguintes bits:\n");
...
void imprime_comum_arq(FILE *arq, Operacao parametros, int saida[]) /* Imprime os dados da soma dos dois operandos, sem cortes, em um arquivo de saida. */
{
    ...
    fprintf(arq,"%d", calcula_decimal(parametros.n1,parametros.bits));
    fprintf(arq," + ");
    fprintf(arq,"%d", calcula_decimal(parametros.n2,parametros.bits));
    fprintf(arq," = ");
    fprintf(arq,"%d", calcula_decimal(saida,parametros.bits));

    fprintf(arq,"\n\n");
}

void analise_mudaporta(Operacao parametros, int codigo_porta) /* Realiza a soma mudando a porta logica XOR do RCA por portas OR ou AND. */
{
    int saida_normal[parametros.bits], saida_nova[parametros.bits];
    ...
    if (codigo_porta == MUDANCA_OR)
        novocircuitoOR(parametros,saida_nova);

    else if (codigo_porta == MUDANCA_AND)
        novocircuitoAND(parametros,saida_nova);
...
void gera_texto(Operacao parametros, int contador_carry) /* Gera o arquivo texto contendo somas com todas os cortes de CARRY IN possiveis. */
{
    ...
    if (!(arq = fopen("cortes_somas.txt","a")))
    {
        system("cls");
        printf("Erro na criacao ou abertura do arquivo cortes_somas.txt. Fim do programa.\n");
        getch();
        exit(1);
    }
    ...
    for (int linha = 0; linha < total_linhas; linha++) /* Iteracao para imprimir os dados no arquivo texto de saida. */
    {
        ...
        if (linha == 0)
            fprintf(arq,"\nOPERACAO 1: sem cortes de CARRY IN");

        else
        {
            fprintf(arq,"\nOPERACAO %d: cortes nos bits ", linha+1);
        ...
        fprintf(arq,"Erro relativo:\t");
        erro_relativo_arq(arq,novo_decimal,decimal_normal);
        fprintf(arq,"------------------------------\n");
    }

    fclose(arq);

    printf("Arquivo texto de saida ""cortes_somas.txt"" atualizado com sucesso.\n");
    printf("\nPressione qualquer tecla para voltar ao menu anterior.");
    getch();
}
