...
int operacoes_aleatorias_mudaporta(int bits, int codigo_porta) /* Realiza somas com mudanca de porta logica, envolvendo operandos aleatorios, e determina as estatisticas. */
{
	...
	for (int i = 0; i < total_operacoes; i++) /* Iteracao para calculo de media aritmetica. */
    {
        ...
        interf_parcial = calcula_decimal(saida_nova,bits);
    ...
    while (1)
    {
        ...
        switch(tecla) /* Analisa a tecla pressionada, para continuar a execucao do programa. */
        {
            ...
            case 'X':
            {
                ...
                if (!(arq = fopen("somasaleatorias_mudaportas.txt","a")))
                {
                    system("cls");
                    printf("Erro na abertura ou criacao do arquivo somasaleatorias_mudaportas.txt. Fim do programa.\n");
                    getch();
                    exit(1);
                }
                ...
                printf("Arquivo texto de saida ""somasaleatorias_mudaportas.txt"" atualizado com sucesso.\n");
...
void analise_aleatoria_mudaporta() /* Realiza calculos estatisticos com somas envolvendo operandos aleatorios e mudancas de portas logicas. */
{
    ...
    while (continuar)
    {
        ...
        printf("\nEscolha qual porta logica substituira as portas XOR no Ripple Carry Adder:\n");
        printf("\n1 - Porta logica OR\n");
        printf("\n2 - Porta logica AND\n");
        ...
        {
            case '1':
                continuar = operacoes_aleatorias_mudaporta(bits, MUDANCA_OR);
                break;

            case '2':
                continuar = operacoes_aleatorias_mudaporta(bits, MUDANCA_AND);
                break;
