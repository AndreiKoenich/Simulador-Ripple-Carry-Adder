...
typedef struct
{
    ...
    float tempo_simulacao;
    float integral_antes;
    float integral_depois;
...
void converte_binario(int op_1, int op_2, float* vetor, int bits, Operacoes dados)
{
    for (int i = bits-1; i >= 0; i--)
    {
        if (op_1 % 2 == 0)
            ...
            vetor[i] = dados.tensao_alimentacao;
        op_1 = op_1 / 2;
    }
...
void recebe_dados(Operacoes *dados)
{
    ...
    printf("\nDigite o tempo (em nanosegundos) de duracao de cada operacao:\n");
    scanf("%f", &dados->tempo_operacoes);
    printf("\nDigite o tempo (em nanosegundos) de medicao da integral da corrente ANTERIOR a cada ponto critico da soma:\n");
    scanf("%f", &dados->integral_antes);
    printf("\nDigite o tempo (em nanosegundos) de medicao da integral da corrente POSTERIOR a cada ponto critico da soma:\n");
    scanf("%f", &dados->integral_depois);
    printf("\nDigite o tempo (em nanosegundos) de transicao entre cada nivel de tensao:\n");
    scanf("%f", &dados->tempo_transicao);
    printf("\nDigite o tempo (em nanosegundos) total da simulacao:\n");
    scanf("%f", &dados->tempo_simulacao);
...
void gera_saida(char nome_arqsaida[], Operacoes dados, float **ondas, int op_1[], int op_2[])
{
    ...
    fprintf(arq_saida,"Vsupply vdd 0 DC %0.2f\n", dados.tensao_alimentacao);
    for (int i = 0; i < 2*dados.bits; i++)
    {
        ...
        for (int j = 0; j < dados.quantidade_operacoes; j++)
        {
            if (ondas[j][i] != dados.tensao_atual)
            {
                ...
                fprintf(arq_saida,"%0.2fns %0.2fV ", (dados.valor_tempo-dados.tempo_operacoes)+dados.tempo_transicao, ondas[j][i]);
            }

            fprintf(arq_saida,"%0.2fns %0.2fV ", dados.valor_tempo, ondas[j][i]);
    ...
    fprintf(arq_saida,"\n\n*-------------------------------------------------- CALCULOS DE INTEGRAIS DA CORRENTE --------------------------------------------------------");

    fprintf(arq_saida,"\n\n.control");
    fprintf(arq_saida,"\ntran 1p %0.2fn\n\n", dados.tempo_simulacao);

    float atual_integral = 0;
    int i = 1;
    while (1)
    {
        atual_integral += dados.tempo_operacoes;

        if (atual_integral+dados.integral_depois > dados.tempo_simulacao)
            break;
        else
        {
            fprintf(arq_saida,"meas tran soma%d integ i(vsupply) from=%0.1fn to=%0.1fn\n", i,atual_integral-dados.integral_antes,atual_integral+dados.integral_depois);
            i++;
        }
    }
    fprintf(arq_saida,"\n*plot v(A)+6 v(B)+4.5 v(Cin)+3 v(s_AxorBxorCin)+1.5 v(Cout)");
    fprintf(arq_saida,"\n.endc");
    fprintf(arq_saida,"\n.end");
    fclose(arq_saida);
}
...
void ondas_especificas()
{
    ...
    for (int i = 0; i < dados.quantidade_operacoes; i++)
    {
        ...
        if(op_2[i] > valor_maximo)
            op_2[i] = valor_maximo;
        converte_binario(op_1[i],op_2[i],ondas[i],dados.bits, dados);
    }
